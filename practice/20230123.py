...
def group_anagrams(a):
    dict1 = {} # 단어를 수집할 딕셔너리 생성
    for i in a:    # 넘겨받은 모든 단어들을 순회
        dict2 = {} # 현재 조사할 word의 정보를 저장할 dict 생성
        for j in i:# 각 단어를 알파벳 기준으로 순회
            if j in dict2: # 만약 순회중인 알파벳이 현재 단어 정보 dict에 이미 있다면
                dict2[j] += 1 # 해당 알파벳의 개수를 1 증가
            else:
